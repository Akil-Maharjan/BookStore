...
function QuickAction({ to, label, description, variant = 'contained' }) {
  ...
  return (
    <Button component={Link} to={to} variant="text" sx={{ ...baseSx, ...variants[variant] }}>
      <Stack spacing={0.5} alignItems="flex-start" sx={{ width: '100%' }}>
        <Typography
          variant="body1"
          sx={{ fontWeight: 700, color: 'inherit', fontFamily: fonts.heading, fontSize: { xs: '1rem', md: '1.05rem' } }}
        >
          {label}
        </Typography>
        {description && (
          <Typography
            variant="body2"
            sx={{ color: 'inherit', opacity: 0.8, fontFamily: fonts.body, lineHeight: 1.5 }}
          >
            {description}
          </Typography>
        )}
  ...
  if (loading) {
    return (
      <Stack spacing={2} mt={2}>
        {[...Array(3)].map((_, idx) => (
          <Skeleton key={idx} variant="rounded" height={68} animation="wave" sx={{ backgroundColor: 'rgba(148,163,184,0.16)' }} />
        ))}
      </Stack>
    );
  }
          ...
          >
            ...
            >
              <Box>
                ...
                <Typography
                variant="caption"
                sx={{ color: 'rgba(226,232,240,0.65)', fontFamily: fonts.body }}
              >
  ...
  const quickActions = useMemo(
    () => [
      ...
      },
      {
        to: '/cart',
        label: 'Go to cart',
        description: cartItems ? `${cartItems} item${cartItems === 1 ? '' : 's'} waiting` : 'Your cart is currently empty',
        variant: 'outlined',
      },
      {
        to: '/orders',
      ...
      <div className="relative z-10">
        <Stack spacing={4}>
          <Paper sx={{ ...surfaceSx, display: 'flex', flexDirection: 'column', gap: 1.5 }}>
            <Typography
              variant="h4"
              sx={{ color: '#f8fafc', fontWeight: 700, fontFamily: fonts.heading, fontSize: { xs: '2rem', md: '2.25rem' } }}
            >
              Welcome{user ? `, ${user.name}` : ''}!
            </Typography>
            <Typography
              variant="body1"
              sx={{ color: 'rgba(226,232,240,0.75)', fontFamily: fonts.body, maxWidth: 600 }}
            >
          ...
          >
            ...
            <Paper sx={surfaceSx}>
              ...
              <RecentOrders orders={recentOrders} loading={ordersQuery.isLoading} />
              {orders.length > 0 && (
                <Button
                  component={Link}
                  to="/orders"
