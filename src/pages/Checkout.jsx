...
export default function Checkout() {
  ...
  const [khaltiReady, setKhaltiReady] = useState(
    typeof window !== "undefined" && !!window.KhaltiCheckout
  );
  ...
  const subtotal = useMemo(
    () =>
      items.reduce(
        (sum, item) =>
          sum + Number(item.book?.price_npr ?? 0) * Number(item.quantity ?? 0),
        0
      ),
    [items]
  );
  const total = useMemo(
    () => Number(subtotal) + Number(shippingFee ?? 0),
  ...
  useEffect(() => {
    ...
    script.onerror = () =>
      toast.error("Unable to load Khalti payment library.");
  ...
  const submitEsewaPayment = (order) => {
    if (!esewaMerchantCode) {
      toast.error(
        "Missing eSewa merchant code. Set VITE_ESEWA_MERCHANT_CODE in your .env file."
      );
      return;
    }
  ...
  const openKhaltiCheckout = async (order) => {
    if (!khaltiPublicKey) {
      toast.error(
        "Missing Khalti public key. Set VITE_KHALTI_PUBLIC_KEY in your .env file."
      );
      return;
    }
  ...
  const handleEsewaSubmit = async (event) => {
    ...
    if (!fullName.trim() || !phone.trim() || !email.trim()) {
      setPaymentError(
        "Please provide your full name, phone number, and email to continue with eSewa."
      );
      return;
    }
    setPaymentError("");
  ...
  const handleKhaltiSubmit = async (event) => {
    ...
    if (!fullName.trim() || !phone.trim() || !email.trim()) {
      setPaymentError(
        "Please provide your full name, phone number, and email to continue with Khalti."
      );
      return;
    }
    setPaymentError("");
      ...
      <div className="relative z-10 mx-auto flex w-full  flex-col gap-10 px-4 sm:px-6 lg:px-8">
        <div className="flex flex-col gap-3 text-white">
          ...
          <p className="text-sm text-slate-300 sm:text-base">
            Review your order details, pick a delivery preference, and choose
                ...
                >
                  <div className="flex flex-col border-b border-white/10 pb-2 pr-4">
                    ...
                    <Typography variant="caption" color="white">
                      ...
                      {formatCurrency(item.book.price_npr || 0)}
                    </Typography>
                  </div>
                  <Typography variant="subtitle1">
          ...
          >
            <div className="flex h-full flex-col justify-between gap-6 text-white">
              ...
              <div className="rounded-2xl border border-white/10 bg-slate-900/40 p-4 sm:p-5">
                {paymentMethod === "esewa" && (
                  <form
                    ...
                    className="space-y-4 font-inter text-slate-100"
                  >
                    ...
                    {paymentError && (
                      <p className="text-sm font-inter text-rose-300">
                        {paymentError}
                      </p>
                    )}
                    <button
                ...
                {paymentMethod === "cod" && (
                  <div className="space-y-4 font-inter text-slate-200">
                    <p className="text-sm">
                      Receive your books first and pay in cash upon delivery.
                      Our support team will reach out to confirm your order and
                      delivery window.
                    </p>
                    {paymentError && (
                      <p className="text-sm font-inter text-rose-300">
              ...
              <div className="rounded-2xl border border-emerald-400/40 bg-emerald-400/10 p-4 font-inter text-sm text-emerald-100">
                <p>
                  Payments are processed using trusted Nepali gateways. Your
                  sensitive details stay secure, and you can always switch
                  methods before confirming the order.
                </p>
              </div>
            </div>
          </Paper>
