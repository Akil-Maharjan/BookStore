...
export default function Orders() {
  ...
  const { data: orders = [], isLoading, isError, error } = useQuery({
    ...
    queryFn: () => getMyOrders(),
    onError: (err) => {
      toast.error(err?.response?.data?.message || 'Failed to load orders', { duration: TOAST_DURATION });
    },
  });
  ...
  const statusMut = useMutation({
    ...
    onError: (err) => {
      toast.error(err?.response?.data?.message || 'Failed to update order', { duration: TOAST_DURATION });
    },
  });
  ...
  const deleteMut = useMutation({
    ...
    onError: (err) => {
      toast.error(err?.response?.data?.message || 'Failed to delete order', { duration: TOAST_DURATION });
    },
  });
  ...
  const filteredOrders = useMemo(() => {
    ...
    return sortedOrders.filter((order) => {
      ...
      const itemTitles = (order.items || []).map((i) => i.book?.title || i.title || '').join(' ').toLowerCase();
          ...
          >
            ...
            >
              ...
              >
                <TableHead>
                  <TableRow>
                    <TableCell sx={{ color: palette.headerText, fontWeight: 600 }}>Order</TableCell>
                    <TableCell sx={{ color: palette.headerText, fontWeight: 600 }}>Placed</TableCell>
                    <TableCell sx={{ color: palette.headerText, fontWeight: 600 }}>Items</TableCell>
                    <TableCell sx={{ color: palette.headerText, fontWeight: 600 }}>Total</TableCell>
                    <TableCell sx={{ color: palette.headerText, fontWeight: 600 }}>Status</TableCell>
                    <TableCell align="right" sx={{ color: palette.headerText, fontWeight: 600 }}>
                      Actions
                    </TableCell>
                  </TableRow>
                </TableHead>
                <TableBody>
                  {displayedOrders.map((order) => {
                    ...
                    return (
                      ...
                      >
                        <TableCell>
                          <Typography variant="subtitle2" fontWeight={600} sx={{ color: palette.primaryText }}>
