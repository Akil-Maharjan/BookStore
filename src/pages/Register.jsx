  ...
  return (
    <div className="max-w-md mx-auto px-4 py-12">
      ...
      <div className="rounded-xl border border-white/10 bg-slate-900 p-6 shadow-card relative z-10">
        ...
        {error && (
          <div className="mb-3 rounded bg-red-500/20 border border-red-500/40 text-red-100 px-3 py-2 text-sm">{error}</div>
        )}
        <form onSubmit={handleSubmit} className="space-y-3">
          <div>
            <label className="block text-sm mb-1">Name</label>
            <input className="w-full rounded border border-white/20 bg-white/10 px-3 py-2 focus:outline-none focus:ring-2 focus:ring-brand" value={name} onChange={(e) => setName(e.target.value)} required />
          </div>
          <div>
            <label className="block text-sm mb-1">Email</label>
            <input className="w-full rounded border border-white/20 bg-white/10 px-3 py-2 focus:outline-none focus:ring-2 focus:ring-brand" type="email" value={email} onChange={(e) => setEmail(e.target.value)} required />
          </div>
          ...
          <button type="submit" disabled={loading} className="w-full rounded bg-brand hover:bg-brand-dark text-white py-2">
            {loading ? 'Creating...' : 'Create Account'}
          </button>
        </form>
        <p className="text-sm text-white/70 mt-3">Already have an account? <Link className="text-brand" to="/login">Sign in</Link></p>
      </div>
    </div>
  );
}
